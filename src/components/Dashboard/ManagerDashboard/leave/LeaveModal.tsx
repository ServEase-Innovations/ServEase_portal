    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fadeIn">
      <div className={`${tc.bgCard} rounded-2xl ${tc.border} ${tc.shadow} max-w-lg w-full max-h-[90vh] overflow-y-auto p-4 sm:p-6`}>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className={`block text-sm font-medium ${tc.text} mb-1.5`}>
              Reason {leaveRequest.reason.length < 5 && <span className="text-xs text-amber-400">(min. 5 characters)</span>}
            </label>
            <textarea
              value={leaveRequest.reason}
              onChange={(e) => setLeaveRequest({ ...leaveRequest, reason: e.target.value })}
              placeholder="Please provide a reason for your leave (minimum 5 characters)..."
              rows={3}
            ...
            <p className={`text-[10px] sm:text-xs ${tc.textMuted} mt-1`}>
              {leaveRequest.reason.length}/1000 characters
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-2">
            <button
              ...
              className={`w-full sm:w-auto px-4 py-2 ${tc.border} ${tc.textSecondary} rounded-xl text-sm font-medium ${tc.bgTableHover} transition-colors ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              Cancel
            </button>
            <button
              ...
              disabled={isSubmitting}
              className={`w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-xl text-sm font-medium hover:from-indigo-600 hover:to-indigo-700 transition-all duration-300 shadow-lg shadow-indigo-500/25 flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              {isSubmitting ? (
                <>
