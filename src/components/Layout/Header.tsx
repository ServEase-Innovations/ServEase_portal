// Header.tsx - Updated with proper spacing and no overlap
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  const getThemeClasses = () => {
    ...
    return {
      header: 'bg-gradient-to-r from-blue-50 via-white to-blue-50 border-b border-blue-100/50',
      text: 'text-gray-800',
  ...
  return (
    <header className={`${themeClasses.header} px-4 sm:px-6 py-2 sm:py-3 transition-colors duration-300`}>
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          ...
          {title ? (
            <div className="flex items-center gap-1 sm:gap-3 min-w-0 flex-1">
              <h1 className={`text-base sm:text-lg md:text-xl font-bold ${themeClasses.text} truncate`}>
                {title}
              </h1>
              {subtitle && (
                <p className={`text-xs sm:text-sm ${themeClasses.textSecondary} hidden md:block truncate max-w-[200px] lg:max-w-none`}>
                  {subtitle}
                </p>
              )}
            </div>
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
              <Link to="/dashboard" className="text-xl sm:text-2xl font-bold text-indigo-500 whitespace-nowrap">
                ServEase
              </Link>
        ...
        <div className="flex items-center gap-1 sm:gap-2 md:gap-3 flex-shrink-0">
          ...
          {!isProfilePage && !isSettingsPage && (
            <div className={`hidden md:flex items-center ${themeClasses.searchBg} rounded-xl px-2 sm:px-3 py-1.5 sm:py-2 border ${themeClasses.searchBorder} transition-all focus-within:ring-2 focus-within:ring-indigo-500/50 max-w-[150px] lg:max-w-[200px]`}>
              <MagnifyingGlassIcon className={`w-3 h-3 sm:w-4 sm:h-4 ${themeClasses.icon} flex-shrink-0`} />
              <input
                type="text"
                placeholder="Search..."
                className={`bg-transparent outline-none text-xs sm:text-sm w-20 md:w-28 lg:w-48 ml-1.5 sm:ml-2 ${themeClasses.searchText} placeholder:${themeClasses.searchPlaceholder} min-w-[60px]`}
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                aria-label="Search"
              />
              <kbd className={`hidden lg:block ml-1.5 sm:ml-2 px-1.5 py-0.5 text-[10px] ${themeClasses.searchBg} rounded border ${themeClasses.searchBorder} ${themeClasses.textSecondary} flex-shrink-0`}>
                ⌘K
              </kbd>
            </div>
          )}
          ...
          <button
            type="button"
            className={`p-1.5 sm:p-2 rounded-xl ${themeClasses.hover} transition-all duration-300 relative group hidden sm:block flex-shrink-0`}
          ...
          {user && (
            <Menu as="div" className="relative flex-shrink-0">
              <Menu.Button 
                className={`flex items-center gap-1 sm:gap-2 md:gap-3 ${themeClasses.hover} rounded-xl px-2 sm:px-3 py-1.5 sm:py-2 transition-all duration-300 border ${themeClasses.searchBorder}`}
              ...
              >
                <div className="relative flex-shrink-0">
                  ...
                  />
                  <span className="absolute bottom-0 right-0 w-2 h-2 sm:w-2.5 sm:h-2.5 bg-green-400 rounded-full ring-2 ring-white dark:ring-[#1a2744]"></span>
                </div>
                <div className="hidden sm:block text-left min-w-0">
                  <p className={`text-xs sm:text-sm font-semibold ${themeClasses.text} truncate max-w-[80px] md:max-w-[120px]`}>
                    {user.name}
                  </p>
                  <p className={`text-[10px] sm:text-xs ${themeClasses.textSecondary} capitalize truncate max-w-[80px] md:max-w-[120px]`}>
                    {user.role.replace('-', ' ')}
                  </p>
                </div>
                <ChevronDownIcon className={`w-3 h-3 sm:w-4 sm:h-4 ${themeClasses.icon} flex-shrink-0`} />
              </Menu.Button>
              ...
              >
                <Menu.Items className={`absolute right-0 mt-2 w-56 sm:w-64 ${themeClasses.dropdownBg} rounded-2xl shadow-2xl border ${themeClasses.dropdownBorder} py-1 z-50 overflow-hidden`}>
                  <Menu.Item>
                    {({ active }) => (
                      <div className={clsx(
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <Link
                        ...
                        className={clsx(
                          `flex items-center px-4 py-2 sm:py-2.5 text-sm ${themeClasses.dropdownText}`,
                          active && themeClasses.dropdownHover
                        )}
                      >
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      <button
                        ...
                        className={clsx(
                          `flex items-center w-full px-4 py-2 sm:py-2.5 text-sm text-red-400 border-t ${themeClasses.divider}`,
