// Header.tsx - Fixed with proper spacing and no overlap
...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className={`${themeClasses.header} px-3 sm:px-4 md:px-6 py-2 sm:py-3 transition-colors duration-300 relative z-10`}>
      <div className="flex items-center justify-between gap-1 sm:gap-2 md:gap-3">
        ...
        <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2 lg:gap-3 flex-shrink-0 flex-wrap justify-end">
          
          {/* Search Bar */}
          {!isProfilePage && !isSettingsPage && (
            <div className={`hidden sm:flex items-center ${themeClasses.searchBg} rounded-xl px-2 sm:px-2.5 md:px-3 py-1 sm:py-1.5 md:py-2 border ${themeClasses.searchBorder} transition-all focus-within:ring-2 focus-within:ring-indigo-500/50 max-w-[100px] sm:max-w-[130px] md:max-w-[160px] lg:max-w-[200px]`}>
              <MagnifyingGlassIcon className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${themeClasses.icon} flex-shrink-0`} />
              <input
                type="text"
                placeholder="Search..."
                className={`bg-transparent outline-none text-xs sm:text-sm w-16 sm:w-20 md:w-24 lg:w-32 ml-1 sm:ml-1.5 md:ml-2 ${themeClasses.searchText} placeholder:${themeClasses.searchPlaceholder} min-w-[50px]`}
                value={searchQuery}
          ...
          >
            {theme === 'dark' ? (
              <SunIcon className={`w-4 h-4 sm:w-4.5 sm:h-4.5 ${themeClasses.icon} group-hover:scale-110 transition-transform`} />
            ) : (
              <MoonIcon className={`w-4 h-4 sm:w-4.5 sm:h-4.5 ${themeClasses.icon} group-hover:scale-110 transition-transform`} />
            )}
          </button>
          ...
          >
            <BellIcon className={`w-4 h-4 sm:w-4.5 sm:h-4.5 ${themeClasses.icon} group-hover:scale-110 transition-transform`} />
            {notifications > 0 && (
          ...
          <button
            ...
            title="View messages"
          >
            <ChatBubbleLeftRightIcon className={`w-4 h-4 sm:w-4.5 sm:h-4.5 ${themeClasses.icon} group-hover:scale-110 transition-transform`} />
          </button>
          ...
          {user && (
            <Menu as="div" className="relative flex-shrink-0">
              {({ open }) => (
                <>
                  <Menu.Button 
                    className={`flex items-center gap-1 sm:gap-1.5 md:gap-2 ${themeClasses.hover} rounded-xl px-1.5 sm:px-2.5 md:px-3 py-1 sm:py-1.5 md:py-2 transition-all duration-300 border ${themeClasses.searchBorder}`}
                  ...
                  >
                    ...
                    <div className="hidden sm:block text-left min-w-0">
                      <p className={`text-xs sm:text-sm font-semibold ${themeClasses.text} truncate max-w-[60px] sm:max-w-[80px] md:max-w-[100px] lg:max-w-[120px]`}>
                        {user.name}
                      </p>
                      <p className={`text-[10px] sm:text-xs ${themeClasses.textSecondary} capitalize truncate max-w-[60px] sm:max-w-[80px] md:max-w-[100px] lg:max-w-[120px]`}>
